...
===== eMachination Prompt-Rig transcript session started: $(date -Is) =====
...
_write_registry_record() {
  printf '{"transcript_path":"%s","pid_path":"%s","terminal_profile":"%s","launcher":"start_transcript_terminal.sh"}\n' \
    "$TRANSCRIPT_PATH" "$TRANSCRIPT_PID_PATH" "$SELECTED_PROFILE" \
    >> "$REGISTRY_PATH"
}

TRANSCRIPT_COMMAND="cd '$REPO_ROOT' && exec script -a -f -c 'VEYDRANET_INNER_PID_PATH='$TRANSCRIPT_PID_PATH'; printf \"%s\n\" \"\$\$\" > \"\$VEYDRANET_INNER_PID_PATH\"; exec bash -i' '$TRANSCRIPT_PATH'"

INNER_COMMAND="$TRANSCRIPT_COMMAND"
